...
public class BoletoCaelum {
	...
	public void boletoModeloNovo(Receita r, com.green.modelo.Boleto boletoPadrao) {
		Banco banco = null;
		Calendar vencimento = Calendar.getInstance();
		vencimento.setTime(r.getDTVencimento());
		Calendar emissao = Calendar.getInstance();
		emissao.setTime(r.getDTEmissao());
		Datas datas = Datas.novasDatas().comDocumento(emissao)
		...
		Emissor emissor = Emissor
				.novoEmissor()
				.comCedente(boletoPadrao.getIDConta().getTitular())
				.comAgencia(boletoPadrao.getIDConta().getAgencia())
				.comDigitoAgencia(boletoPadrao.getIDConta().getAgenciaDigito())
				.comContaCorrente(boletoPadrao.getIDConta().getNumero())
				.comDigitoContaCorrente(
						boletoPadrao.getIDConta().getNumeroDigito())
				.comNossoNumero(
						leftPadWithZeros(r.getIDReceita().toString(), 11))// gerado
																			// pela
																			// empresa
																			// nao
																			// se
																			...
																			// mesmo
																			// cliente
				.comDigitoNossoNumero("1").comCarteira("9");

		switch (boletoPadrao.getIDConta().getIDBanco().getCodigo()) {
		// Banco do Brasil
		case "001":
		...
		Sacado sacado = Sacado
				.novoSacado()
				.comNome(r.getIDCliente().getIDPessoa().getRazao())
				...
				.comEndereco(
						r.getIDCliente().getIDPessoa().getLogradouro()
								+ " Nº:"
								+ r.getIDCliente().getIDPessoa().getNumero()
								+ " , "
								+ r.getIDCliente().getIDPessoa()
										.getComplemento());
		...
		Boleto boleto = Boleto
				...
				.comNumeroDoDocumento(
						r.getIdorigem().getIDContratoMidia().getNContrato()
								+ " -- "
								+ r.getNumero()
								+ "/"
								+ r.getIdorigem().getIDContratoMidia()
										.getNumeroParcelas())
				.comInstrucoes(
						"** VALORES EXPRESSOS EM REAIS",
						"MORA DIA / COM.PERMANENC.........."
								+ boletoPadrao.getMora(),
						"APÓS " + r.getDTVencimento().getDate() + "/"
								+ (r.getDTVencimento().getMonth() + 1) + "/"
								+ (r.getDTVencimento().getYear() + 1900)
								+ "  MULTA ........" + boletoPadrao.getMulta())
				.comLocaisDePagamento(
						"Todos os bancos até o vencimento ,após vencimento somente "
								+ boletoPadrao.getIDConta().getIDBanco()
										.getDescricao());
		...
		try {
			JasperReport subrelatorioJasper = (JasperReport) JRLoader
					.loadObject(context
							.getRealPath("relatorio/boleto-default_instrucoes.jasper"));
			parametros.put("SUB_INSTRUCOES", subrelatorioJasper);
			JRDataSource jrds = new JRBeanCollectionDataSource(
					Arrays.asList(boleto));
			RelatorioUtil.geraRelatorioBean("boleto-default", jrds, parametros);
		} catch (JRException ex) {
			Logger.getLogger(BoletoCaelum.class.getName()).log(Level.SEVERE,
					null, ex);
		}
	}
	...
	public static void boletoModelo(Receita r) {
		...
		Boleto boleto = Boleto
				...
				.comLocaisDePagamento(
						"Todos os bancos até o vencimento ,após vencimento somente "
								+ r.getIDConta().getIDBanco().getDescricao());
