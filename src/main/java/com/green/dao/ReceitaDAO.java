...
public class ReceitaDAO extends AbstractDao<Receita, Integer> {
	...
	public Receita carregar(Integer k) {
		Query query = getSf()
				.getCurrentSession()
				.createQuery(
						"from com.green.modelo.Receita r where r.iDReceita = :id ")
				.setInteger("id", k);
	...
	public List<Receita> listar() {
		...
		receitas = getSf().getCurrentSession().createCriteria(Receita.class)
				.list();
		return receitas;
	}
	...
	public List<Receita> parcelasCliente(Date inicio, Date fim, Cliente c) {
		Query query = getSf()
				.getCurrentSession()
				.createQuery(
						" from com.green.modelo.Receita d  "
								+ "where d.dTVencimento >= :inicio and d.dTVencimento <= :fim "
	...
	public List<Receita> receitaPorPeriodo(Date inicio ,Date fim,boolean ativo){
		Query query = getSf().getCurrentSession().createQuery("from com.green.modelo.Receita d  "
								+ "where d.dTVencimento >= :inicio and d.dTVencimento <= :fim and d.ativo = :ativo"
								+ " order by d.iDCliente.iDPessoa.razao").setDate("inicio", inicio).setDate("fim",fim).setBoolean("ativo", ativo);
		return query.list();
	}
	public List<Receita> todasReceitaPorPeriodo(Date inicio ,Date fim){
		Query query = getSf().getCurrentSession().createQuery("from com.green.modelo.Receita d  "
								+ "where d.dTVencimento >= :inicio and d.dTVencimento <= :fim "
								+ " order by d.iDCliente.iDPessoa.razao").setDate("inicio", inicio).setDate("fim",fim);
		return query.list();
	}
			

	public List<Receita> filtro(Receita receitaFiltro, Date fimVenc, int pag,
			BigDecimal valorIni, BigDecimal valorFim) {
		...
		if (receitaFiltro.getDTVencimento() == null && fimVenc == null) {
			...
			Date dataMesAntes = new Date(d2.get(Calendar.YEAR) - 1900,
			...
			if (receitaFiltro.getDTVencimento() != null && fimVenc != null) {
				...
				sql = sql
						+ " d.dTVencimento >= :inicio and d.dTVencimento <= :fim";
			} else if (receitaFiltro.getDTVencimento() != null
					&& fimVenc == null) {
				params.put("inicio", receitaFiltro.getDTVencimento());
		...
		if (!valorFim.equals(new BigDecimal("0.00"))
				|| !valorIni.equals(new BigDecimal("0.00"))) {
			params.put("valorIni", valorIni);
			params.put("valorFim", valorFim);
			sql = sql
					+ " and d.valorLiquido >= :valorIni and d.valorLiquido <= :valorFim";
		}
		query = getSf()
				.getCurrentSession()
				.createQuery(
						sql
								+ " and d.ativo = true order by d.IDConta,d.dTVencimento")
	...
	public List<Receita> vencimentosDoDia() {
		...
		Date dataMesAntes = new Date(d2.get(Calendar.YEAR) - 1900,
				d2.get(Calendar.MONTH), 1);
		d2.set(Calendar.DAY_OF_MONTH,
				d2.getActualMaximum(Calendar.DAY_OF_MONTH));
		Query query = getSf()
				.getCurrentSession()
				.createQuery(
						"From com.green.modelo.Receita d where d.dTVencimento<= :ultimo and d.dTVencimento>=:d2 order by d.dTVencimento")
				.setParameter("d2", dataMesAntes)
				.setParameter("ultimo", d2.getTime());
	...
	public List<Receita> liberacaoPagamento() {
		Query query = getSf()
				.getCurrentSession()
				.createQuery(
						"From com.green.modelo.Receita d where d.pago = 0 and d.atzPg = 1 order by d.dTVencimento ");
		return (List<Receita>) query.list();
	}
	...
	public List<Receita> pendentesPagamento() {
		Query query = getSf()
				.getCurrentSession()
				.createQuery(
						"From com.green.modelo.Receita d where d.pago = 0 and d.atzPg = 2 order by d.dTVencimento");
		return (List<Receita>) query.list();
	...
	public List<Receita> pendentesPagamentoAteHoje() {
		...
		Query query = getSf()
				.getCurrentSession()
				.createQuery(
						"From com.green.modelo.Receita d where d.pago = 0 and d.dTVencimento < :data and d.ativo = true order by d.dTVencimento ")
				.setDate("data", d);
		return (List<Receita>) query.list();
	...
	public List<Receita> pagamentoProximoPendenteCliente() {
		...
		Query query = getSf()
				.getCurrentSession()
				.createQuery(
						"From com.green.modelo.Receita d where d.pago = 0 "
								+ "and d.dTVencimento < :data and d.iDCliente.iDTipocliente is null "
								+ "and d.idorigem.IDContratoMidia.iDtipopagamento.descricao != 'Contra Apresentação' order by d.dTVencimento ")
				.setDate("data", c.getTime());
		return (List<Receita>) query.list();
		...
		Query query = getSf()
				...
				.createQuery(
						"From com.green.modelo.Receita d where d.pago = 0 "
								...
								+ " d.ativo = true order by d.dTVencimento , d.iDCliente ");
		return (List<Receita>) query.list();
		...
		Query query = getSf()
				...
				.createQuery(
						"From com.green.modelo.Receita d where d.pago = 0 "
								...
								+ " and d.ativo = true order by d.dTVencimento , d.iDCliente ");
		return (List<Receita>) query.list();

	}

	public List<Receita> pagamentoProximoPendenteClienteFlitro(Date inicio,
			Date fim) {
		...
		Query query = getSf()
				...
				.createQuery(
						"From com.green.modelo.Receita d where d.pago = 0 "
								+ " and d.iDCliente.iDTipocliente is null "
								+ "and d.idorigem.IDContratoMidia.iDtipopagamento.descricao != 'Contra Apresentação'"
								+ " and d.dTVencimento >= :inicio and d.dTVencimento <= :fim order by d.dTVencimento ")
				.setDate("inicio", inicio).setDate("fim", fim);
		return (List<Receita>) query.list();
	...
	public List<Receita> pagamentoPendenteParceiroFiltro(Date inicio, Date fim) {
		Query query = getSf()
				...
				.createQuery(
						"From com.green.modelo.Receita d where d.pago = 0 "
								+ " and d.iDCliente.iDTipocliente is not null and d.dTVencimento >= :inicio and d.dTVencimento <= :fim "
	...
	public List<Receita> pagamentoPendenteContraApresentacaoFiltro(Date inicio,
			Date fim) {
		...
		Query query = getSf()
				...
				.createQuery(
						"From com.green.modelo.Receita d where d.pago = 0 "
								+ " and d.idorigem.IDContratoMidia.iDtipopagamento.descricao = 'Contra Apresentação' "
								+ " and d.dTVencimento >= :inicio and d.dTVencimento <= :fim "
								+ " order by d.dTVencimento , d.iDCliente ")
				.setDate("inicio", inicio).setDate("fim", fim);
		return (List<Receita>) query.list();
	}
	public List<Receita> contaReceberEmAberto(Date inicio ,Date fim){
		Query query = getSf()
				.getCurrentSession()
				.createQuery(
						"From com.green.modelo.Receita d where d.pago = :pg "
								+ " and d.dTVencimento >= :inicio and d.dTVencimento <= :fim "
								+ " order by d.dTVencimento , d.iDCliente ")
				.setDate("inicio", inicio).setDate("fim", fim).setBoolean("pg", false);
		return (List<Receita>) query.list();
	}
}
