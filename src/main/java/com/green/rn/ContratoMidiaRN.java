...
public class ContratoMidiaRN {
	...
	// novo cadastro de contratos atualizado em 20/11/2014
	@Transactional("tmGreen")
	public void cadastrarNovo(ContratoMidia contratoMidia, Date inicio, boolean producao, List<Painel> paineis) {

		Receita receitaContrato = new Receita();
		if (paineis != null) {
			contratoMidia.getPracas().clear();
			for (Painel p : paineis) {
				contratoMidia.getPracas().add(p.getIDPraca());
			}
		}
		try {
			...
			if(paineis!=null){
				for (Painel painel : paineis) {
					this.painelContratoDAO.salvar(new PainelContrato(contratoMidia,painel));
				}
			}
	 ...
	 * e envia o status inicial para a producão começar a divulgar o video. só
	 * messa nessa funcão se tiver plenos conhecimentos do que esta fazendo
	 */
	@Transactional("tmGreen")
	public boolean salvar(ContratoMidia contratoMidia, Contato contato, List<Praca> pracas, Mailing mailing, boolean tipo, boolean producao, boolean inicio) {
		...
		try {
			...
			if (contextoBean.getUsuarioLogado().getIDGrupoAcesso().getDescricao().equals("ROLE_ADMINISTRACAO")
				...
				// referente a inclusão do contrato en receitas paramentros
				// fixos ja pre estabelecidos
			...
			FacesContext context = FacesContext.getCurrentInstance();
			context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_FATAL, "Falha na trasmissão de dados para o banco!",
					"Falha na trasmissão de dados para o banco!"));
			return false;
		}
	}
	...
	public void salvarParceiro(ContratoMidia midia, BigDecimal valorReceita, List<Praca> pracas, boolean produzir) {
		...
		try {
			...
			FacesContext.getCurrentInstance()
					.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Contrato salvo com sucesso!", "Contrato salvo com sucesso!"));
			...
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_FATAL, "Ocorreu erro ao salvar o contrato!", "Ocorreu erro ao salvar o contrato!"));
	...
	public List<ReceitaPraca> receitaPorPracaValorMensal(Praca praca, Calendar periodo) {
		...
		return getContratoMidiaDAO().TESTEreceitaPorPracaValorMensal(praca, periodo);
	...
	/*
	 * Calcula parcelas contando do dia inicial da ate o final gerando paracela
	 * com pro-rata do primeiro mês e do ultimo mês gerando as parcelas
	 * intermediarias do valor integral de cada parcela
	 */
	...
	private Map<Date, BigDecimal> calculaProRata(Date inicio, Date fim, BigDecimal valor) {
		...
		int ultimoDiaMesInicio = dataInicio.getActualMaximum(Calendar.DAY_OF_MONTH);// ultimo
																					// dia
																					// do
																					// mes
																					// de
																					// inicio
		int ultimoDiaMesFim = dataFim.getActualMaximum(Calendar.DAY_OF_MONTH); // ultimo
																				// dia
																				// do
																				// mes
																				// de
																				// termino
		int prorataInicio = (ultimoDiaMesInicio - dataInicio.get(Calendar.DAY_OF_MONTH)) + 1;// quantos
																								// dias
																								// serão
																								// cobrados
																								// do
																								...
																								// de
																								// inicio
		int prorataFim = dataFim.get(Calendar.DAY_OF_MONTH);// quantos dias seão
															// cobrados do mês
															// de termino
		...
		int numDias = (int) ((((result / 1000) / 60) / 60) / 24) + 1; // calcular
																		// numero
																		// total
																		// de
																		// dias
		...
		if (numDias <= 31) { // menos de 31 dias pegara o valor e dividira se
								// for em dois meses e se o periodo for dentro
								// do mesmo mês ira gerar apenas uma parcela com
								// o valor integral
			...
			primeiraParcela = primeiraParcela.multiply(new BigDecimal(prorataInicio));// calculo
																						// do
																						// valor
																						// da
																						// pro-rata
			...
			dataInicio.add(Calendar.MONTH, +1);
			map.put(dataInicio.getTime(), primeiraParcela);// salvando mês
															// evalor da
															// primeira parcela
			// dataFim.roll(Calendar.MONTH, false);
			while (dataInicio.before(dataFim)) {// laço para gera as parcelas
												// intermediarias ao periodo.
			...
			map.put(d, ultimaParcela);// calculo e guardando mês e valor
										// pro-rata da ultima parcela
	...
	@Transactional(value = "tmGreen")
	public void cancelar(ContratoMidia contratoMidia) {
		contratoMidia.setAtivo(0);
		for (Receita receita : contratoMidia.getOrigen().getReceitas()) {
			if (!receita.getPago()) {
		...
		getContratoMidiaDAO().atualizar(contratoMidia);
		FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Cancelado com sucesso!", null));
	}

	public List<ContratoMidia> buscarRazaoLike(String razao) {
		return getContratoMidiaDAO().buscarRazaoLike(razao);
	}

	@Transactional(value = "tmGreen")
	public void ativar(ContratoMidia contratoMidia) {
		contratoMidia.setAtivo(1);
		for (Receita receita : contratoMidia.getOrigen().getReceitas()) {
			if (!receita.getPago()) {
				receita.setAtivo(true);
		...
		getContratoMidiaDAO().atualizar(contratoMidia);
		FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Ativado com sucesso!", null));
	}
