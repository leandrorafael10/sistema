        ...
        switch (codigo) {
            ...
            case 2: {
                /*
                 ...
                 JRDataSource jrds = new JRBeanCollectionDataSource(getReceitacreditoRN().filtro(r, dataFim, 1, new BigDecimal("0.00"), new BigDecimal("0.00")));
                 RelatorioUtil.geraRelatorioBean("parcelas_recebidas", jrds, parametros);
                 */
                Receita r = new Receita();
                r.setDTVencimento(getDataInicio());
                List<Receita> receitas = getReceitaRN().filtro(r, dataFim, 1, new BigDecimal("0.00"), new BigDecimal("0.00"));
                HashMap<String, Object> parametros = new HashMap<>();
                parametros.put("Usuario", ContextoUtil.getContextoBean().getUsuarioLogado().getIDFuncionario().getIDPessoa().getRazao());
                parametros.put("DataInicio", getDataInicio());
                parametros.put("DataFim", getDataFim());
                parametros.put("Subtitulo", "Relatório parcelas já quitadas");
                JRDataSource jrds = new JRBeanCollectionDataSource(receitas);
                RelatorioUtil.geraRelatorioBean("parcelas_a_receber", jrds, parametros);
            }
            break;
            ...
            case 9: {
                JRDataSource jrds = new JRBeanCollectionDataSource(getReceitaRN().parcelasCliente(dataInicio, dataFim, this.receita.getIDCliente(), getTipo()));
                HashMap<String, Object> parametros = new HashMap<>();
                parametros.put("usuario", ContextoUtil.getContextoBean().getUsuarioLogado().getIDFuncionario().getIDPessoa().getRazao());
                parametros.put("dataInicio", getDataInicio());
                parametros.put("dataFim", getDataFim());
                parametros.put("cliente", receita.getIDCliente().getIDPessoa().getRazao());
                RelatorioUtil.geraRelatorioBean("parcelas_a_receber_cliente", jrds, parametros);
            }
            break;
            ...
            case 10: {
                ...
                parametros.put("tipo", (getStatus() == 1 ? "ativo" : (getStatus() == 0 ? "cancelado" : "todos")));
                JRDataSource jrds = new JRBeanCollectionDataSource(getContratoMidiaRN().buscaPorPeriodoTipo(dataInicio, dataFim, status));
                RelatorioUtil.geraRelatorioBean("contratos_periodo_tipo", jrds, parametros);
            }
            break;
            ...
            case 11: {
                ...
                parametros.put("dataInicio", getDataInicio());
                parametros.put("dataFim", getDataFim());
                parametros.put("parceiro", getReceita().getIDCliente().getIDPessoa().getFantasia());
                parametros.put("titulo", " CONTRATOS PARCEIRO ");
                parametros.put("tipo", (getStatus() == 1 ? "ativo" : (getStatus() == 0 ? "cancelado" : "todos")));
                JRDataSource jrds = new JRBeanCollectionDataSource(getContratoParceiroRN().buscaPorPeriodoTipo_parceiro(dataInicio, dataFim, status, getReceita().getIDCliente()));
                RelatorioUtil.geraRelatorioBean("contratos_periodo_tipo_parceiro", jrds, parametros);
            }
            break;
            //
            case 12:
            ...
            case 15: {
                Receita r = new Receita();
                r.setDTVencimento(getDataInicio());
                List<Receitacredito> receitas_pagas = getReceitacreditoRN().listaReceitaPorVendedor(dataInicio, dataFim, contratoMidia.getIDvendedor());
                HashMap<String, Object> parametros = new HashMap<>();
                parametros.put("usuario", ContextoUtil.getContextoBean().getUsuarioLogado().getIDFuncionario().getIDPessoa().getRazao());
                ...
                parametros.put("vendedor", contratoMidia.getIDvendedor().getIDPessoa().getRazao());
                JRDataSource jrds = new JRBeanCollectionDataSource(receitas_pagas);
                RelatorioUtil.geraRelatorioBean("comissao_vendedor", jrds, parametros);
            }
            break;
            //
            case 16:
